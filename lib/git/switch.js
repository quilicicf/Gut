  ...
  return {
    switch: (yargs) => {
      ...
      if (toMaster) {
        ...
        try {
          const featureBranch = branches.getBranchParent(branchDescription, branches.BRANCH_TYPES.FEATURE);
          execution.execute(`git checkout ${featureBranch}`);
        } catch (error) {
          execution.exit(1, `Can't find the feature version from ${branches.getCurrentBranch()}.`);
        }

      } else if (toBase) {
        execution.execute(`git checkout ${branches.getBranchParent(branchDescription)}`);
