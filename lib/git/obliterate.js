...
const deleteItem = async (itemName, itemToDelete, targetRemoteArgument) => {
  ...
  const message = `Delete ${itemName} ${itemToDelete.bold} on ${targetRemote.name}?`;
  const shouldAbort = await prompt.yesNoPrompt({ message, defaultValue: false });
  if (shouldAbort) {
