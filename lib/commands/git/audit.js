...
const describe = 'Audits a given diff';
...
// FIXME: mutates the input :scream:
...
const parseLine = (seed, line) => {
  ...
  if (REGEX_LINE_NEW_FILE.test(line)) {
    _.set(newSeed, [ 'current', 'fileName' ], REGEX_LINE_NEW_FILE.exec(line)[ 2 ]);
  ...
  if (REGEX_LINE_START_DIFF.test(line)) {
    ...
    _.set(newSeed, [ 'current', 'oldLineNumber' ], parsedLine[ 1 ]);
    _.set(newSeed, [ 'current', 'newLineNumber' ], parsedLine[ 4 ]);
  }
...
const auditArgs = yargs => yargs
  ...
  .option(ARG_FROM.name, ARG_FROM)
  .option(ARG_TO.name, ARG_TO)
  .coerce(ARG_COMMITS_NUMBER.name, (argument) => {
    if (argument < 1) {
...
const auditHandler = (args) => {
  const commitsNumber = args[ ARG_COMMITS_NUMBER.name ];
  const fromBaseBranch = args[ ARG_FROM_BASE_BRANCH.name ];
  const from = args[ ARG_FROM.name ];
  const to = args[ ARG_TO.name ] || 'HEAD'; // TODO: should be a default but yargs fails on conflicts rule if set
  ...
  if (commitsNumber) {
    ...
    const diff = execution.execute('git --no-pager diff HEAD -U0 --no-color');
    parseDiffAndDisplay(diff);
  }
};
