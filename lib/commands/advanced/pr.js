...
const createPullRequest = async (assigneeArgument, baseBranch) => {
  const parsedRepositoryPath = configuration.parseRepositoryPath();
  const repositoryName = configuration.getRepositoryNameFromRemote();
  ...
  if (shouldWriteDescription) {
    ...
    const initialContent = existsSync(prTemplateFile) ? readFileSync(prTemplateFile, 'utf8') : '';

    const fileToEdit = path.resolve(os.tmpdir(), `gut_pr_${branches.getCurrentBranchName()}_description.md`);

    writeFileSync(fileToEdit, initialContent);
    await execution.spawnAsync(process.env.EDITOR, [ fileToEdit ], { stdio: [ 'inherit', 'inherit', 'ignore' ] });

    _.set(prOptions, 'description', readFileSync(fileToEdit, 'utf8'));
  }
...
const prArgs = (yargs) => yargs.usage(`usage: gut ${command} [options]`)
  ...
  .option(ARG_BASE_BRANCH.name, ARG_BASE_BRANCH)
  .help();

const prHandler = async (args) => {
  const {
    [ ARG_OPEN_PR.name ]: shouldOpenPrInDefaultBrowser,
    [ ARG_COPY_PR_URL.name ]: shouldCopyPrUrlInClipboard,
    [ ARG_ASSIGNEE.name ]: prAssigneeFromCli,
    [ ARG_BASE_BRANCH.name ]: baseBranchFromCli,
  } = args;

  const baseBranch = baseBranchFromCli || branches.getBranchDescription().baseBranch;
  const branchOnlyCommits = branches.getCommitsFrom(baseBranch);
  ...
  const prUrl = await createPullRequest(prAssigneeFromCli, baseBranch);
  if (shouldOpenPrInDefaultBrowser) { await execution.openFile(prUrl); }

  if (shouldCopyPrUrlInClipboard) {
