module.exports = (() => {
  ...
  const parseBranchName = branchName => {
    const branchRegex = /^([0-9]+\.[0-9]+\.[0-9]+(\.[0-9]+)?)(_([^#]+)?#([^_]+))(_([0-9]+))(_([^_]+))$/;
    ...
    const parsedBranch = {
      ...
      feature: branchFragments[ 5 ] || '',
      ticketNumber: branchFragments[ 7 ] || '',
      description: branchFragments[ 9 ] || '',
      isBuildable: branchFragments[ 4 ] === 'build'
    };
