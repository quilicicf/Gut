...
module.exports = (() => {
  ...
  return {
    audit: (yargs, os) => {
      ...
      if (commitsNumber) {
        diff = execution.execute(`git --no-pager diff -U0 --no-color HEAD~${commitsNumber}..HEAD`);
        ...
        diff = execution.execute(`git --no-pager diff -U0 --no-color ${from}..${to}`);
      } else {
        diff = execution.execute('git --no-pager diff -U0 --no-color');
      }
