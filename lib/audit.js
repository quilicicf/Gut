module.exports = (() => {
  ...
  const ARGUMENTS = {
    COMMITS_NUMBER: {
      ...
      type: 'integer',
      conflicts: [ 'from', 'to', 'from-base-branch' ]
    },
    FROM_BASE_BRANCH: {
      name: 'from-base-branch',
      alias: 'b',
      describe: 'Shows an audit of all commits that are not on the base branch',
      type: 'boolean',
      conflicts: [ 'commits-number', 'from', 'to' ]
    },
    FROM: {
      name: 'from',
      alias: 'f',
  ...
  return {
    audit: (yargs, os) => {
      ...
      const to = arguments[ ARGUMENTS.TO.name ];
      if (commitsNumber) {
