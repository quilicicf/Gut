...
const GIT_SERVERS_PRESET = {
  github: {
    getRepositoryUrl: (owner, repository) => `git@github.com:${owner}/${repository}.git`,
  },
};
...
const getRepositoryOption = (optionPath, defaultValue = undefined) => {
...
const getRepositoryNameFromRemote = () => {
  const remoteDescription = execSync('git remote show -n origin', { encoding: 'utf8' });
  return /Fetch URL: git@[^:]+:[^/]+\/(.*?)\.git/m.exec(remoteDescription)[ 1 ];
};
