...
module.exports = (() => {
  ...
  const searchForLocalBranch = (regex) => {
    const allBranches = execution.execute('git branch 2> /dev/null | grep -v -F \'*\'');
    return _(allBranches.split('\n'))
      .map(branch => _.replace(branch, /^[* ] /, ''))
  ...
  const getMaxCommitNumber = () => {
    return parseInt(execution.execute('git --no-pager log -n 50 --pretty=format:\'%H\' | wc -w'), 0);
  };

  const getBranchOnlyCommits = (maxCommitNumber) => {
    const numberOfCommitsToInspect = maxCommitNumber || getMaxCommitNumber();
  ...
  const getBranchParent = (parsedBranch, targetBranchType) => {
    ...
    return targetBranchType && baseBranch.type !== targetBranchType
