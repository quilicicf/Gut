...
const BRANCH_TYPES = {
  VERSION: {
    name: 'version',
    matches: parsedBranch => _.isEmpty(parsedBranch.feature) && _.isEmpty(parsedBranch.name),
  ...
  DEV: {
    name: 'dev',
    matches: parsedBranch => !_.isEmpty(parsedBranch.name),
    getBaseBranch: parsedBranch => ({
      version: parsedBranch.version,
...
const buildBranchName = (parsedBranch) => {
  ...
  const descriptionFragment = parsedBranch.name || '';
...
/**
 * Returns the number of commits that are present on this branch but not its direct parent.
 */
