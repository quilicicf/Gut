module.exports = (() => {
  ...
  const ARGUMENTS = {
    FORMAT: {
      ...
      // TODO: should work ? Doesn't. Remove the associated coerce call
      // options: _.keys(LOG_FORMATS),
      type: 'string',
      default: 'pretty'
    },
    SKIP: {
  ...
  return {
    history: yargs => {
      const arguments = yargs
        ...
        .coerce(ARGUMENTS.FORMAT.name, argument => {
          if (!_.has(LOG_FORMATS, argument)) {
            const availableOptions = `[ ${_.keys(LOG_FORMATS)} ]`.bold;
            throw Error(`No format corresponding to ${argument.bold}. Choose the format in ${availableOptions}`.red);
          }
