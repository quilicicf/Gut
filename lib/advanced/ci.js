...
module.exports = {
  ci: async () => {
    ...
    if (jobsSize === 0) {
      ...
      const shouldRun = await prompt.yesNoPrompt({ message: `Do you want to run job ${jobKey.cyan}?`, defaultValue: true });
      if (shouldRun) {
        executeJob(job, toolConfigurations, accounts);
      }

    } else {
      const jobToRun = await prompt.chooseFromList('Which job do you want to run?\n', _.keys(allJobs));
