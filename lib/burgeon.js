...
module.exports = (() => {
  ...
  const createBranch = (newParsedBranch, gutOptions) => {
    ...
    newBranchDescription.baseBranch = branches.getCurrentBranch();

    const newBranchName = branches.buildBranchName(newParsedBranch);

    execution.execute(`git checkout -b ${newBranchName}`);
    execution.execute(`git config branch.${newBranchName}.description '${JSON.stringify(newParsedBranch)}'`);
  };
  ...
  const createFeatureBranch = (parsedBranch, args, gutOptions) => {
    ...
    if (!utils.isMasterBranch(parsedBranch) && !branches.isVersionBranch(parsedBranch)) {
