...
const gutOptionsPath = utils.GLOBAL_OPTIONS_FILE_PATH;
let gutOptionsPromise;
try {
  fs.statSync(gutOptionsPath);
  gutOptionsPromise = new Promise((resolve) => {
    resolve(JSON.parse(fs.readFileSync(gutOptionsPath, 'utf8')));
  });
} catch (err) {
  gutOptionsPromise = require('./lib/init.js').init(gutOptionsPath);
}
...
gutOptionsPromise.then(gutOptions => {
  yargs
